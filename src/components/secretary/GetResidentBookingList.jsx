...
import { useCreateBookingBillMutation, useLazyGetResidentBookingQuery } from '@/redux/api/secreatry.api';
import { toast } from 'react-toastify';

const GetResidentBookingList = () => {

    const [getBookingFacility, { data, isLoading }] = useLazyGetResidentBookingQuery();

    const [generateBill, { isSuccess }] = useCreateBookingBillMutation()
                                    ...
                                    <tr key={item._id} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-4 py-3 text-center text-md">
                                            <div className="flex justify-center items-center h-full w-full gap-5">
                                                <Button
                                                    className="border border-[#145EFB] text-[#145EFB] bg-transparent hover:bg-[#145EFB] hover:text-white  cursor-pointer px-4"
                                                    onClick={() => generateBill(item._id)}
                                                    disabled={item.billGenerated === true}
                                                >
                                                    {item.billGenerated ? "Bill Generated" : "Generate Bill"}
                                                </Button >

                                                <FiEye
                                                    className="text-blue-600 cursor-pointer text-lg"
                                                    onClick={() =>
                                                        navigate(`/secretary/facility/booking/details/${item._id}`)
                                                    }
                                                />
                                            </div>
                                        </td>
...
export default GetResidentBookingList;
