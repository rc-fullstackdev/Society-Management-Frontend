...
import { useLazyGetResidentGuestQuery } from '@/redux/api/residential.api';

const ResidentGuestList = () => {

    const [getResidentGuest, { data, isLoading }] = useLazyGetResidentGuestQuery();
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-[#145EFB]">
                                <tr>
                                    ...
                                    <th className="px-4 py-3 text-center text-md font-medium text-white">Vehicle Number</th>
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {guest.map((item) => (
                                    <tr key={item._id} className="hover:bg-gray-50">
                                        <td className="px-2 py-3 text-center text-md">{item.guestName}</td>
                                        <td className="px-2 py-3 text-center text-md">{item.mobileNumber}</td>
                                        <td className="px-2 py-3 text-center text-md">{formatDate(item.createdAt)}</td>

                                        <td className="px-2 py-3 text-center text-md">
                                            {item.vehicleNumber}
                                        </td>
...
export default ResidentGuestList;
